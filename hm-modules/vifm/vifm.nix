{
  ...
  cfg = config.custom.vifm;
in {
  options.custom = {
    vifm.enable = mkEnableOption "Enable vifm hm-module";
  };
  ...
  config = mkIf cfg.enable {
    ...
    xdg.configFile."vifm/vifmrc" = {
      text =
        # notice: I had to escape a double single quote in here
        /*
        vifm
        */
        ''
                 ...
                 colorscheme nord Default-256 Default
          ...
          " Directories (self-added)
          fileviewer */,.*/ ${pkgs.tree}/bin/tree -all -F --filelimit 20 -L 3 -C  %c

                 " Pdf
                 filextype {*.pdf},<application/pdf> ${pkgs.zathura}/bin/zathura %c %i, apvlv %c, xpdf %c
